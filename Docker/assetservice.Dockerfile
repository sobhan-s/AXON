...
RUN turbo prune @dam/assetService --docker
...
COPY --from=builder /app/apps/assetService/dist ./apps/assetService/dist
COPY --from=builder /app/apps/assetService/package.json ./apps/assetService/package.json
COPY --from=builder /app/packages ./packages
COPY --from=builder /app/node_modules ./node_modules
